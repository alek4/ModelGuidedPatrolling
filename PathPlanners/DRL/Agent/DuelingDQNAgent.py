...
from PathPlanners.DRL.ReplayBuffers.ReplayBuffers import PrioritizedReplayBuffer, ReplayBuffer
from PathPlanners.DRL.Networks.network import DuelingVisualNetwork, NoisyDuelingVisualNetwork, \
	DistributionalVisualNetwork
import torch.nn.functional as F
from tqdm import trange
from PathPlanners.DRL.ActionMasking.ActionMaskingUtils import NoGoBackMasking, SafeActionMasking, \
	ConsensusSafeActionMasking
import time
...
class MultiAgentDuelingDQNAgent:
	...
	def __init__(
		...
		self.store_only_random_agent = store_only_random_agent
		""" Logging parameters """
		np.random.seed(seed)
		self.logdir = logdir
		...
		self.safe_masking_module = SafeActionMasking(action_space_dim=action_dim,
		                                             movement_length=self.env.movement_length)
		self.nogoback_masking_modules = {i: NoGoBackMasking() for i in range(self.env.number_of_agents)}
		self.consensus_masking_module = ConsensusSafeActionMasking(self.env.scenario_map, action_space_dim=action_dim,
		                                                           movement_length=self.env.movement_length)
	...
	def select_masked_action(self, states: dict, positions: dict, deterministic: bool = False):
		...
		for agent_id, state in states.items():
			...
			self.safe_masking_module.update_state(position=positions[agent_id],
			                                      new_navigation_map=self.env.scenario_map)
	...
	def train(self, episodes, write_log=True, verbose=True):
		...
		for episode in trange(self.episode, initial_episode + int(episodes)):
			...
			while not all(done.values()):
				...
				if self.store_only_random_agent:
					
					selected_agent_id = np.random.choice(list(next_state.keys()))
					
					self.transition = [state[selected_agent_id],
					                   actions[selected_agent_id],
					                   reward[selected_agent_id],
					                   next_state[selected_agent_id],
					                   done[selected_agent_id],
					                   {}]
