...
class algae_bloom:
    ...
    def reset(self):
        ...
        for starting_point in starting_points:
            ...
            # If the self.particles are not initialized, we initialize them
            if self.particles is None:
                self.particles = np.random.multivariate_normal(starting_point, cov=np.array((6+4*np.random.rand())*np.eye(2)), size=(np.random.randint(0,20) + 80,))
            else:
                self.particles = np.vstack((
                    self.particles,
                    np.random.multivariate_normal(starting_point, cov=np.array((6+4*np.random.rand())*np.eye(2)), size=(np.random.randint(0,20) + 80,))))
